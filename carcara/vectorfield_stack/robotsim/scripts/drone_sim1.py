...
class drone_node(object):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if (self.trajectory_stage.data != 0 and self.trajectory_stage.data != 3 and self.trajectory_stage.data != 5 and self.trajectory_stage.data != 7):
                ...
                #Model integration
                self.state[0] = self.state[0] + 2*vel_w[0]*time_step/1.5
                self.state[1] = self.state[1] + 2*vel_w[1]*time_step/1.5
                self.state[2] = self.state[2] + 2*vel_w[2]*time_step/1.5
                ...
                self.state[7] = self.state[7] + 2*acc[0]*time_step/1.5
                self.state[8] = self.state[8] + 2*acc[1]*time_step/1.5
                self.state[9] = self.state[9] + 2*acc[2]*time_step/1.5
    ...
    def init_node(self):
        ...
        self.pos_subscriber = rospy.Subscriber("/drone/odom2",  Odometry, self.odometry_cb2)
