...
class drone_node(object):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if (self.trajectory_stage.data != 0 and self.trajectory_stage.data != 3 and self.trajectory_stage.data != 7 and self.trajectory_stage.data != 5):
                ...
                # Adjust velocities during maneuver
                # Model integration

                factorr = (5 - self.first_y)/5

                self.state[0] = self.state[0] + factorr*vel_w[0]*time_step
                self.state[1] = self.state[1] + factorr*vel_w[1]*time_step
                self.state[2] = self.state[2] + factorr*vel_w[2]*time_step
                ...
                self.state[7] = self.state[7] + factorr*acc[0]*time_step
                self.state[8] = self.state[8] + factorr*acc[1]*time_step
                self.state[9] = self.state[9] + factorr*acc[2]*time_step

            else:
                pass

            #print(f"MELECA{self.first_y}")

            #Publis robots velocities
            collect_msg =  Float64MultiArray()
            dados = [vel_w, quat_dot]
            collect_msg.data = dados


            self.pub_leo.publish(collect_msg)
    ...
    def init_node(self):
        ...
        for i in range(self.number_of_robots):
            ...
            self.posr.append([0,0,0])
            self.obtscles_r.append(0.2)
        ...
        self.pub_leo = rospy.Publisher(f"/collect/leo{self.robot_number}",  Float64MultiArray, queue_size=100)
