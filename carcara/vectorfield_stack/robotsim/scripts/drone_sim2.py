...
class drone_node(object):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if (self.trajectory_stage.data != 0 and self.trajectory_stage.data != 3 and self.trajectory_stage.data != 5 and self.trajectory_stage.data != 7):
                #Model integration
                ...
                #Model integration
                self.state[0] = self.state[0] + vel_w[0]*time_step/1.5
                self.state[1] = self.state[1] + vel_w[1]*time_step/1.5
                self.state[2] = self.state[2] + vel_w[2]*time_step/1.5
                ...
                self.state[5] = q_new[2]
                self.state[6] = q_new[3]
                print("MELEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEECA")
                
                self.state[7] = self.state[7] + acc[0]*time_step/1.5
                self.state[8] = self.state[8] + acc[1]*time_step/1.5
                self.state[9] = self.state[9] + acc[2]*time_step/1.5
            ...
            if (count2 == 4):
                ...
                self.obtscles_pos = [self.posr1+self.posr2]
                n_obst = len(self.obtscles_pos)
                D_close = float("inf")
                o_close = 0
    ...
    def init_node(self):
        ...
        # publishers
        self.pub_pose = rospy.Publisher(f"/drone/pose{self.robot_number}", Pose, queue_size=1)
        self.pub_odom = rospy.Publisher(f"/drone/odom{self.robot_number}", Odometry, queue_size=1)
        self.pub_closest_world = rospy.Publisher(f"/drone/closest_point_world{self.robot_number}", Point, queue_size=1)
        self.pub_closest_body = rospy.Publisher(f"/drone/closest_point_body{self.robot_number}", Point, queue_size=1)
        self.pub_rviz_robot = rospy.Publisher(f"/drone/robot{self.robot_number}", MarkerArray, queue_size=1)
        self.pub_rviz_closest = rospy.Publisher(f"/drone/closest_marker{self.robot_number}", Marker, queue_size=1)
        self.pub_rviz_obst = rospy.Publisher(f"/drone/obstacles{self.robot_number}", MarkerArray, queue_size=1)
        self.pub_rviz_hist = rospy.Publisher(f"/drone/history{self.robot_number}", MarkerArray, queue_size=1)


        # self.pub_rviz_robot_w1 = rospy.Publisher("/drone/robot_w1", Marker, queue_size=1)

        # subscribers
        rospy.Subscriber("/drone/acrorate" + f"{self.robot_number}", Quaternion, self.callback_acrorate)
        rospy.Subscriber("/trajectory_state" + f"{self.robot_number}",  Float32, self.trajectory_state_cb)
        self.pos_subscriber = rospy.Subscriber("/drone/odom0",  Odometry, self.odometry_cb1)
        self.pos_subscriber = rospy.Subscriber("/drone/odom1",  Odometry, self.odometry_cb2)
