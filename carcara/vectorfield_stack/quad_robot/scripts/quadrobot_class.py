...
class quadrobot_class():
    ...
    def __init__(self, vr, kf, reverse_direction, flag_follow_obstacle, epsilon, switch_dist_0, switch_dist, m, Kv, Kw,robot_number):
        ...
        self.pos_subscriber = rospy.Subscriber("/trajectory_state"+ f"{self.robot_number}",  Float32, self.stage_cb)
    ...
    def control_step_parallel(self):
        ...
        [a_r,J] = self.get_acc_ref(pos,vel);
        a_r_M = self.get_acc_ref(pos_M,vel_M)[0];
        a_r_m = self.get_acc_ref(pos_m,vel_m)[0];
        # a_r = [0,0,9.81]
        # a_r_M = [0,0,9.81]
        # a_r_m = [0,0,9.81]
    ...
    def control_step(self):
        ...
        self.pub_tau_vel = rospy.Publisher("/collect/tau",  Float64MultiArray, queue_size=100)

        collect_msg =  Float64MultiArray()

        dados = a_r + z_b + [self.tau] + J[0] + J[1] +J[2]

        #Publis robots velocities
        collect_msg.data = dados

        self.pub_tau_vel.publish(collect_msg)
