...
class Paths:
    ...
    def __init__(self):
        ...
        rospy.init_node("path_generator1", anonymous=True)
    ...
    def refference_path_3(self,N,posi):
        ...
        for k in range(N):
            ...
            # Compute a point of the ellipse in a local frame
            x_ref0 = 2*self.a * cos(p)
            y_ref0 = 2*self.b * sin(p)
    ...
    def path(self):
        ...
        while not rospy.is_shutdown():
            ...
            # Centralize Above Fire Stage
                ...
                aux+=2
                self.pos1 = self.pos
                self.pos_des1 = np.array(self.pos_des)+np.array([0.8,0,0])
                self.path2 = self.refference_path_2(self.number_of_samples,self.pos,self.pos_des1)
                ...
                self.got_there = 0
                x = np.linalg.norm(np.array(self.pos) - np.array(self.pos_des1))
                if (x <= 0.2):
                    self.got_there = 1
                    self.etapa += 1
                self.pub_state.publish(self.etapa)
