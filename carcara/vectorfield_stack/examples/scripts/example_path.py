...
class Paths:
    ...
    def refference_path_3(self,N,posi):
        ...
        for k in range(N):
            ...
            # Compute a point of the ellipse in a local frame
            x_ref0 = 1.5*self.a * cos(p)
            y_ref0 = 1.5*self.b * sin(p)
    ...
    def path(self):
        ...
        while not rospy.is_shutdown():
            ...
            # Centralize Above Fire Stage
                ...
                aux+=2
                self.pos1 = self.pos
                self.path2 = self.refference_path_2(self.number_of_samples,self.pos,self.pos_des)
                ...
                self.got_there = 0
                x = np.linalg.norm(np.array(self.pos) - np.array(self.pos_des))
                if (x <= 0.2):
                    self.got_there = 1
                    self.etapa += 1
                self.pub_state.publish(self.etapa)
            ...
            # Wait for reunion
                ...
                y = np.linalg.norm(np.array(self.pos) - np.array(self.posr2))
                if (x <= 1 and y <= 1):
                    self.reunited = 1
